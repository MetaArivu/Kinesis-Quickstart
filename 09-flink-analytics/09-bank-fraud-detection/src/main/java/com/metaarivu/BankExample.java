...
public class BankExample {
	...
	private static final String path = "/Users/ketangote/workspace_learning/kinesis-quickstart/09-flink-analytics/09-bank-fraud-detection/src/main/resources/";
	...
	public static void main(String[] args) throws Exception {
		...
		DataStream<AlarmedCustomer> alarmedCustomers = env
				.readTextFile(path+"alarmed_cust.txt")
		...
		DataStream<LostCard> lostCards = env
				.readTextFile(path+"lost_cards.txt")
	...
	public static class Citychange extends ProcessWindowFunction<Tuple2<String, String>, Tuple2<String, String>, String, TimeWindow> {
		...
		public void process(String key, Context context, Iterable<Tuple2<String, String>> input, Collector<Tuple2<String, String>> out) {
			...
			for (Tuple2<String, String> element : input) {
				...
				if (changeCount >= 2) {
					out.collect(new Tuple2<String, String>("Alarm->", element + "marked for FREQUENT city changes"));
				}
			}
		}
	}
	...
	public static class FilterAndMapMoreThan10 implements FlatMapFunction<Tuple3<String, String, Integer>, Tuple2<String, String>> {
		...
		public void flatMap(Tuple3<String, String, Integer> value, Collector<Tuple2<String, String>> out) {
			if (value.f2 > 10) {
				out.collect(new Tuple2<String, String>("Alarm->", value + " marked for >10 TXNs"));
			}
		}
		...
		public void processElement(Tuple2<String, String> value, ReadOnlyContext ctx,
			...
			for (Map.Entry<String, AlarmedCustomer> custEntry : ctx.getBroadcastState(alarmedCustStateDescriptor)
				...
				if (tId.equals(alarmedCustId)) {
					out.collect(new Tuple2<String, String>("Alarm->",
		...
		public void processElement(Tuple2<String, String> value, ReadOnlyContext ctx,
			...
			for (Map.Entry<String, LostCard> cardEntry : ctx.getBroadcastState(lostCardStateDescriptor).immutableEntries()) {
				...
				if (cId.equals(lostCardId)) {
					out.collect(
							new Tuple2<String, String>("Alarm->", "Transaction: " + value + " issued via LOST card"));
				}
			}
		}
